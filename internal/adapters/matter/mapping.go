...
func nodeToDiscovered(n Node) ports.DiscoveredDevice {
	features := featuresFromNode(n)
	return ports.DiscoveredDevice{
		TransportRef: domain.TransportRef(n.NodeID),
		Type:         deviceTypeFor(features),
		Name:         displayNameFor(n),
		Manufacturer: n.VendorName,
		Model:        n.ProductName,
		Features:     features,
		Metadata:     map[string]string{"matter.nodeId": n.NodeID},
	}
}
...
// MVP behaviour — good enough for single-function devices; multi-endpoint
// devices will get proper per-endpoint feature grouping in a follow-up.
func featuresFromNode(n Node) []domain.Feature {
...
// deviceTypeFor picks the coarse-grained DeviceType from the feature set. A
// device with color/dimming is a light; only OnOff is a switch/plug (we lean
// on switch as safer default — plugs typically also carry power metering).
func deviceTypeFor(features []domain.Feature) domain.DeviceType {
...
func actionToInvoke(ref domain.TransportRef, feature domain.FeatureKey, action domain.ActionKey, params map[string]any) (InvokeParams, error) {
	base := InvokeParams{NodeID: string(ref), EndpointID: defaultEndpoint}
	switch feature {
